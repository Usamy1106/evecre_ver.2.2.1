 ...
 * リアクションのトグル。
 *
 * ★MongoDB 側で原子的に更新すること。以前は findOne → JS で加工 → reactions を丸ごと
 *   $set しており、2人がほぼ同時に押すと後勝ちで片方のリアクションが消えていた
 *   （エラーも出ないので気づけない）。意気込みの🔥（server.js の motivation-reactions）と
 *   同じ方針に揃えてある。
 */
async function toggleReaction(msgId, emoji, userId) {
  // 絵文字をフィールド名に使うので、パスを壊す文字を弾く（'.' はサブフィールド区切り）
  if (typeof emoji !== 'string' || !emoji || emoji.includes('.') || emoji.startsWith('$')) return null;
  const path = `reactions.${emoji}`;

  const doc = await col().findOne({ _id: msgId });
  if (!doc) return null;
  const has = (doc.reactions?.[emoji] || []).includes(userId);

  if (has) {
    // 取り消し。$pull は冪等なので、同時に押されても二重に消えたりしない
    await col().updateOne({ _id: msgId }, { $pull: { [path]: userId } });
    // 誰もいなくなったらキーごと消す（空配列のチップを出さないため）。
    // 条件付きなので、この間に誰かが押していれば消えない
    await col().updateOne({ _id: msgId, [path]: { $size: 0 } }, { $unset: { [path]: '' } });
  } else {
    // $addToSet は userId が素の文字列なので重複しない（＝二度押しでも増えない）
    await col().updateOne({ _id: msgId }, { $addToSet: { [path]: userId } });
  }

  const fresh = await col().findOne({ _id: msgId });
