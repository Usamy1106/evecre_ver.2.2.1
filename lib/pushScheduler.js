...
async function _collectTargets() {
  ...
  for (const ev of events) {
    ...
    const eventLite = {
      ...
      // ★フェーズと完了フラグ。片付けが済んだイベントに通知を送らないための判定に使う
      //   （wrapup_review ルール）。CRDT セルなので fields 経由で取ること。
      eventPhase:  ev.fields?.eventPhase?.v ?? null,
      isCompleted: ev.fields?.isCompleted?.v === true,
    };
...
async function runSlot(slot, opts = {}) {
  ...
  if (slotRules.length === 0) {
    return { slot, dryRun, error: 'no_rules', targets: 0, sent: 0, skipped: 0, failed: 0, noSub: 0, expired: 0 };
  }
  ...
  const summary = {
    ...
    // ★「通知を許可していない人」を failed に混ぜない（ログを見て送信障害と誤読しないため）。
    //   noSub  … 購読ゼロ＝そもそも送る先が無い（通知未許可・PWA 未追加など。異常ではない）
    //   expired… 端末側で購読が失効しており 404/410 で掃除された（これも異常ではない）
    noSub: 0, expired: 0,
    byRule: {},
    ...
    for (const uid of chunkIds) {
      ...
      for (const rule of slotRules) {
        ...
        try {
          ...
          // sendPushToUser は購読ゼロ（＝送り先なし）でも {0,0,0} を返す。
          // 「1件でも送れた／実際に失敗した／失効を掃除した／そもそも購読が無い」を区別する。
          if      (r.sent    > 0) summary.sent++;
          else if (r.failed  > 0) summary.failed++;
          else if (r.expired > 0) summary.expired++;
          else                    summary.noSub++;
        } catch (e) {
          summary.failed++;
  ...
  console.log(
    ...
    `スキップ=${summary.skipped} 未購読=${summary.noSub} 失効=${summary.expired} ` +
    `失敗=${summary.failed} ${summary.ms}ms ` +
