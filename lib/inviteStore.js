...
// { _id: token(hex), eventId, createdBy, createdAt, expiresAt: Date, maxUses, usedBy: [] }
 ...
 * - 使用上限を超えているものも除外。
 */
async function listInvitesForProject(eventId) {
  const now = new Date();
...
/** イベントに紐づく全招待を削除（イベント削除時のクリーンアップ） */
async function deleteForProject(eventId) {
  if (!eventId) return;
