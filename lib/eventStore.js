 ...
 *
 * 参加申請フォーム（joinFormModal）が入る前から居るメンバーは skillsGood / skillsWant を
 * 持たないため、担当者のおすすめが機能しない。イベントを開いたときにモーダルで聞き、
 * ここで members[] に直接書き込む（members は CRDT 対象外なので applyPatch を通さない）。
 *
 * ★回収が済んだらこの関数ごと削除すること（呼び出し元も一緒に消す）。
 *
 * @param {string} eventId
 * @param {string} userId
 * @param {{skillsGood: string[], skillsWant: string[], joinedAnswersAt: number}} answers
...
module.exports = {
  ...
  setFolderId,
  listByFolder,
  setMemberJoinAnswers,   // ★暫定（既存メンバーのスキル回収）。回収後に削除する
  // 同期ヘルパ
  isMember,
  getRole,
  getMemberRoleIds,
