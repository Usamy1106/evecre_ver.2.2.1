...
async function loadEvent(eventId) {
  ...
  // ★条件は isLegacyShape に1本化してある（PUT /api/data のフォールバック判定と同一にするため）
  if (isLegacyShape(p)) {
...
/**
 * 旧フラット形式のドキュメントかどうか。
 * ★loadEvent の昇格判定はこの関数を使う。条件を2箇所にコピーしないこと
 *   （片方だけ直したときに静かに壊れる）。
 * ★loadEvent を経ずに取得したドキュメント（listEventsForUser / loadEvents の戻り値）を
 *   書き込み経路へ渡す前に、これで確認する。server.js の PUT /api/data が使う。
 */
function isLegacyShape(p) {
  return !!p && !p.fields && Array.isArray(p.missions);
}

/**
 * 複数イベントをまとめてロードする。
 * ★SSE 接続時（GET /api/events）が id ごとに loadEvent を回していたのを置き換える。
 *   購読イベント数ぶん往復していた（モバイルは再接続が多く、毎回これが走る）。
 * ★loadEvent と違い、旧形式の昇格は**しない**。呼び出し側が isLegacyShape で判断すること。
 *   SSE 側は isMember（members 配列のみ参照）にしか使わないので旧形式でも結果は変わらない。
 * @param {string[]} eventIds
 * @returns {Promise<object[]>} 見つかったものだけ。順序は保証しない
 */
async function loadEvents(eventIds) {
  if (!Array.isArray(eventIds) || eventIds.length === 0) return [];
  const docs = await col().find({ _id: { $in: eventIds } }).toArray();
...
/**
 * ユーザーが「閲覧のみ」かどうか（書き込み操作を禁止する）。
 *
 * ★管理者権限が優先される。ロールを複数持っていて「閲覧のみ」と「管理者権限」が
 *   両方付いている場合は、管理者権限が勝って閲覧のみにはならない。
 *   権限を弱いほうに倒すと、管理者が自分を締め出して復旧できなくなるため。
 *   UI にもこの優先順位を明示すること（イベント設定のロール編集）。
 * ★判定は必ずこの関数を通すこと。呼び出し側で roles を直接見て回ると、
 *   優先順位の実装が散らばって食い違う。
 */
function isViewOnly(event, userId) {
  if (canManage(event, userId)) return false;   // ★管理者権限が優先
  const ids = getMemberRoleIds(event, userId);
  if (ids.length === 0) return false;
  const roles = getRoles(event);
  return ids.some(rid => {
    const def = roles.find(r => r.id === rid);
    return !!(def && def.viewOnly);
  });
}
...
/**
 * このイベントで「他の団体にも公開してよい」が初めて選ばれたことを記録する（確認モーダルを出し終えた印）。
 * ★CRDT のセル（fields.shareConfirmedAt）に直接書く。クライアントからは書かせない
 *   （一般メンバーも立てるので PUT /api/data の canManage を通れないため。サーバーが代わりに立てる）。
 * ★まだ無いときだけ書く（最初に確認した時刻を残す）。
 * @returns {Promise<boolean>} 今回立てたら true
 */
async function markShareConfirmed(eventId, now = Date.now()) {
  const r = await col().updateOne(
    { _id: eventId, 'fields.shareConfirmedAt': { $exists: false } },
    { $set: { 'fields.shareConfirmedAt': { v: now, t: now } } },
  );
...
/**
 * 自分の「参加時の回答」（できること・やってみたいこと・ひとこと）を members[] に書く。
 * members は CRDT 対象外なので applyPatch を通さず positional `$` で直接更新する。
 *
 * 呼び出し元は2つ：
 *   - イベント設定の「プロフィール設定」（PUT /api/events/:id/my-answers）… 恒久
 *   - ★暫定のスキル回収（POST /api/events/:id/my-skills）… 回収後に呼び出し元だけ消す。
 *     ★この関数は恒久機能でも使うので、一緒に消さないこと。
 *
 * ★joinMessage は渡されたときだけ書く。スキル回収はひとことを聞かないので、
 *   無条件に書くと参加時に書いたひとことを空で上書きしてしまう。
 *
 * @param {string} eventId
 * @param {string} userId
 * @param {{skillsGood: string[], skillsWant: string[], joinMessage?: string, joinedAnswersAt: number}} answers
 * @returns {Promise<boolean>} メンバーが見つかって更新できたか
 */
async function setMemberJoinAnswers(eventId, userId, answers) {
  const set = {
    'members.$.skillsGood':      answers.skillsGood,
    'members.$.skillsWant':      answers.skillsWant,
    'members.$.joinedAnswersAt': answers.joinedAnswersAt,
  };
  if (typeof answers.joinMessage === 'string') set['members.$.joinMessage'] = answers.joinMessage;
  const r = await col().updateOne(
    { _id: eventId, 'members.userId': userId },
    { $set: set }
  );
...
module.exports = {
  // async CRUD
  loadEvent,
  loadEvents,      // 複数イベントを1クエリで（旧形式の昇格はしない）
  saveEvent,
  deleteEvent,
  applyPatch,
  listEventsForUser,
  setFolderId,
  listByFolder,
  setMemberJoinAnswers,   // 参加時の回答の更新（プロフィール設定と暫定のスキル回収で共用）
  markShareConfirmed,     // 「公開してよい」の初回確認を済ませた印（サーバーだけが書く）
  // 同期ヘルパ
  isLegacyShape,   // 旧フラット形式か（loadEvent の昇格条件と同一）
  isMember,
  getRole,
  getMemberRoleIds,
  setMemberRoleIds,
