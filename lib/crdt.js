...
/**
 * 2つのCRDTプロジェクトをマージして新しいプロジェクトを返す
 * 不変メタ情報（id, ownerId, members, createdAt）はサーバー側のものを優先
 */
function mergeProject(server, incoming) {
  ...
  return {
    ...
    roles:     server.roles || incoming.roles || null,  // roles は CRDT 対象外、サーバー側を優先
    customTags: server.customTags || incoming.customTags || null, // customTagsも同様
...
const FLAT_PROJECT_FIELDS = [
  'name', 'description', 'dates', 'seedType', 'isCompleted',
  'progress', 'daysLeft', 'lastProposalClearedTime',
  'likes', 'hasLiked', 'inviteCode',
];
...
const FLAT_PROPOSAL_FIELDS = ['id', 'title', 'tag', 'description'];

const FLAT_CLEARED_FIELDS = ['title', 'summary', 'url', 'venue', 'period', 'image'];

/**
 * フラット形式の project を CRDT 形式に変換
 * @param {object} flat
...
function flatToCrdt(flat, now) {
  const fields = {};
  for (const k of FLAT_PROJECT_FIELDS) {
...
/**
 * CRDT 形式の project をクライアント表示用フラット形式に変換
 * @param {object} crdt
 * @returns {object}
 */
...
/**
 * 古いフラット形式（v0.5まで）のプロジェクトを CRDT 形式へ昇格
...
module.exports = {
  mergeCell, mergeFields, mergeMap, mergeProject,
