...
function mergeEvent(server, incoming) {
  ...
  return {
    ...
    // 意気込みへのリアクション。★CRDT の LWW セルにしてはいけない（配列全体が
    // Last-Write-Wins になり、同時に押した2人のうち1人が消える）。専用エンドポイントの
    // $addToSet / $pull で原子的に更新する。
    motivationReactions: server.motivationReactions || incoming.motivationReactions || [],
...
const FLAT_EVENT_FIELDS = [
  ...
  // イベント作成フローで聞く項目（すべて任意。既存イベントは undefined のまま壊れない）
  // eventType     … 種別。proposalEngine のカテゴリ判定を detectCategory() から置き換える
  // expectedScale … 想定来場者規模。AI プロンプトに渡す（ルールベースの加点には使わない）
  // catchphrase   … 一言説明（「一言で言うとどんなイベント？」。旧キャッチコピー）。招待ページに表示
  // motivationTags / motivationText … 意気込み。1オブジェクトにまとめず2つに分ける
  //   （カードだけ後から編集したときに一言を巻き込まないため）
  'eventType', 'expectedScale', 'catchphrase',
  'motivationTags', 'motivationText',
  // handoverDates … 引き継ぎ（振り返り）を行う日。'YYYY-MM-DD' の配列で複数日可。
  //   ★dates（開催日）とは別に持つ。開催日の計算・ガント・締切表示に混ぜないこと。
  //   最終日の翌日にフェーズが自動で「完了」になる（state.js の _checkEventPhase）。
  'handoverDates',
  // アーカイブの基礎情報（2026-09-26）。以前は提出物（clearedData の 'archive-venue' /
  //   'archive-image'）に持たせていたが、タスクの記録と混ざるのでイベント自身の項目にした。
  //   ★旧データは utils.js の getArchiveVenue / getEventMainVisual が読み替える（移行はしない）
  // venue       … 開催場所（文字列）
  // headerImage … ヘッダー画像（R2 の URL）。★サーバーの _sanitizeEventFields が
  //               「このイベントの R2 画像」以外を捨てる（dataURL・外部 URL を CRDT に入れない）
  'venue', 'headerImage',
  // publicBasicInfo … 基礎情報（上の5つ）を公開するか。true / false。undefined＝まだ聞いていない
  //   （modals/publicBasicInfoModal.js が一度だけ聞く）。★既定は非公開。
  //   ★書けるのは管理者だけ（既存イベントの更新は canManage 必須）。値はサーバーが boolean に限る
  'publicBasicInfo',
  // publicKnowledge  … ナレッジ（タスクの内容・振り返り）を公開するか。★開催日を過ぎてから、管理者だけ。
  //   サーバーが「開催後か」「公開してよい振り返りがあるか」を確かめ、満たさない true は捨てる
  // shareConfirmedAt … このイベントで「他の団体にも公開してよい」の初回確認を済ませた時刻。
  //   ★サーバーだけが書く（eventStore.markShareConfirmed）。クライアントが送っても捨てる
  'publicKnowledge', 'shareConfirmedAt',
  // publicKnowledgePromptedAt … 振り返りフェーズが終わったあと「タスクも含めて公開するか」を聞いた時刻。
  //   （modals/publicKnowledgeModal.js）。★「今はしない」と答えたら入り、このイベントではもう聞かない
  'publicKnowledgePromptedAt',
];
...
function flatToCrdt(flat, now) {
  ...
  return {
    ...
    motivationReactions: flat.motivationReactions || [],
...
function crdtToFlat(crdt) {
  const out = {
    ...
    motivationReactions: crdt.motivationReactions || [],
