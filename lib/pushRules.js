...
/**
 * そのイベントが「振り返りフェーズに入った日」か。
 *
 * ★最終開催日の**ちょうど翌日**だけを真にする。ここを「以降」にすると、
 *   片付けが済むまで毎日「振り返りを記入しよう！」が鳴り続ける
 *   （重複防止キーは日付単位なので、日をまたぐと再送されてしまう）。
 * ★片付けが済んでいるイベント（完了フェーズ／isCompleted）には送らない。
 * ★開催日が未設定のイベントは対象外（判定の基準が無い）。
 *   ★手でフェーズを「振り返り」にした場合はここでは拾えない。日付基準で
 *     自動遷移する経路（state.js の _checkEventPhase）と足並みを揃えている。
 */
function isWrapUpStartDay(ev, todayStr) {
  if (!ev) return false;
  if (ev.isCompleted || ev.eventPhase === '完了') return false;
  const dates = Array.isArray(ev.dates) ? [...ev.dates].filter(Boolean).sort() : [];
  const last = dates.at(-1);
  if (!last) return false;
  return daysOverdue(last, todayStr) === 1;
}
...
const SCHEDULED_RULES = [
  ...
  {
    // 開催が終わった翌朝、振り返りの記入を促す。
    // ★宛先はそのイベントの全メンバー（担当者だけではない）。振り返りは
    //   関わった全員が書くもので、1イベントに一度しか鳴らないため。
    // ★slot を既存と分けているのは exclusive のため。09:00 / 13:00 / 21:45 は
    //   それぞれ1日1通なので、そこへ入れると締め切り通知と食い合う。
    id:        'wrapup_review',
    slot:      '10:00',
    priority:  1,
    enabled:   true,
    exclusive: true,
    label:     '振り返りの記入を促す',
    match: (ctx) => (ctx.events || []).some(ev => isWrapUpStartDay(ev, ctx.todayStr)),
    build: (ctx) => {
      const ev = (ctx.events || []).find(e => isWrapUpStartDay(e, ctx.todayStr));
      return {
        title: ev?.name ? `「${ev.name}」おつかれさまでした` : 'おつかれさまでした',
        body:  '振り返りを記入しよう！',
        // ★イベント単体へのディープリンクは無い（/m/<eventId>/<missionId> のみ）。
        //   ルートに送って通常の復帰に任せる。
        url:   '/',
        tag:   `wrapup-${ev?.id || 'event'}`,
      };
    },
  },
