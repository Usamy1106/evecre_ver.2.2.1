...
/**
 * 「まとめる」通知。同じ userId × type × eventId の**未読**通知があれば
 * 本文と時刻を上書きし、無ければ1件だけ新規作成する。
 *
 * 何度も起きうる出来事（意気込みへのリアクションなど）で使う。addNotification を
 * そのまま使うと押した人数ぶん通知が積み上がり、すぐに鬱陶しくなる。
 * 既読になった後の新しい出来事は、改めて1件作られる（見終わったものは畳まない）。
 *
 * @param {string[]} userIds
 * @param {{ type, message, eventId, missionId?, actorId?, actorName? }} notif
 */
async function upsertEventNotification(userIds, notif) {
  if (!Array.isArray(userIds) || userIds.length === 0) return;
  await Promise.all(userIds.map(async (userId) => {
    const r = await col().updateOne(
      { userId, type: notif.type, eventId: notif.eventId || null, read: false },
      { $set: { message: notif.message || '', createdAt: new Date() } }
    );
    if (r.matchedCount === 0) await addNotification(userId, notif);
  }));
}
