...
// ── 作成フローの eventType → テンプレのカテゴリ ────────────────
// public/js/constants.js の EVENT_TYPES と1対1で対応させること。
// ★選択肢を足したらここにも足す。未定義は 'general' に落ちるので、
//   足し忘れるとカテゴリ固有のテンプレが選ばれなくなる（静かに劣化する）。
//
// festival_market を party に寄せているのは、物販・飲食の運営タスクが必要なため
//（"フェス" は music のキーワードでもあるが、音楽性より出店運営のほうが本質）。
// これに合わせて party カテゴリへ物販・飲食系のテンプレを追加してある。
const EVENT_TYPE_TO_CATEGORY = {
  exhibit:          'exhibit',
  festival_market:  'party',
...
// ★選択肢は 企画準備 / 振り返り / 完了 の3つ（eventSettings.js と必ず一致させること）。
//   かつて「告知」があったが、開催後の自動遷移先として「振り返り」に置き換えた。
//   ★振り返り・完了では提案自体を止めている（state.js の _checkProposalCycle）ので、
//     この写像が実際に効くのは「企画準備」のときだけ。整合のために残してある。
...
function generateProposals({
  name = '',
  description = '',
  eventType = null,        // 作成フローで明示された種別（あればカテゴリ推測より優先）
  expectedScale = null,    // 想定来場者規模。テンプレ側に規模メタが無いのでスコアには使わない
  existingTitles = [],
  usedProposalIds = [],
  eventDates = [],
  daysLeft = null,
  ...
  // 明示された eventType を最優先。無ければ従来どおり名前と説明文から推測する。
  // ★detectCategory は消さないこと（eventType を持たない既存イベントに必要）。
  const category  = eventType
    ? (EVENT_TYPE_TO_CATEGORY[eventType] || 'general')
  ...
  let reserveSpark;
  if (eventPhase === '完了' || eventPhase === '振り返り') reserveSpark = false;
...
module.exports = { generateProposals, detectCategory, detectPhase, PHASES, EVENT_TYPE_TO_CATEGORY };
