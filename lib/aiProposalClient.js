...
function _buildUserPrompt(ctx) {
  ...
  if (ctx.catchphrase)        infoLines.push(`- 一言で言うと: ${ctx.catchphrase}`);
  if (ctx.motivation)         infoLines.push(`- 意気込み: ${ctx.motivation}`);
  if (ctx.description)        infoLines.push(`- 説明: ${ctx.description}`);
  // 初期タスク「どのようなイベントを行うか整理しよう」の提出内容（旧・概要）。★説明とは別に渡す
  if (ctx.planning)           infoLines.push(`- 企画の整理: ${ctx.planning}`);
  infoLines.push(
  ...
  return [
    ...
    '- 【最重要】**イベント名を最優先の手がかり**にしてください。名前に含まれる固有名詞' +
      ...
      + teamLine,
    // ★3件の tag は「1件ずつ別の担当領域」にする。提案ボックスは常に3体の
    //   キャラクター（mizu=運営 / mori=制作・広報 / iwa=企画）が左から順に
    //   1体ずつ並び、体の色がそのままラベル色を表すため（public/js/constants.js の
    //   PROPOSAL_CHARACTERS）。ここが崩れるとキャラの色と提案のタグが食い違う。
    '- tag は3件で必ず**次の3つを1件ずつ**にしてください：' +
      '1件目「運営」、2件目「制作」か「広報」のどちらか、3件目「企画」。' +
      'この3件を必ず揃えること（同じ tag を2件出さない）。',
 ...
 * @param {object} ctx
 *   { name, description, planning, phase, eventPhase, daysLeft,
