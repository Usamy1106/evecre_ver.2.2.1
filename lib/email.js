...
//   - 'resend'   : Resend HTTP API (要 RESEND_API_KEY) ★ Render 推奨
//   - 'gmail'    : Gmail SMTP (要 GMAIL_USER, GMAIL_APP_PASSWORD) ※Render では動作しない
//   - 'sendgrid' : SendGrid SMTP (要 SENDGRID_API_KEY) ※Render では動作しない
...
// Resend HTTP API クライアント（SMTP を使わないため Render でも動作）
...
function _getTransporter() {
  if (_resendClient || _transporter || _initError) return _transporter;
  ...
  try {
    switch (TRANSPORT_KIND) {
      case 'resend': {
        ...
        // HTTP API（SMTP 不使用）— Render 等 SMTP ブロック環境でも動作
...
/**
 * Resend HTTP API でメールを送信する。
 * @returns {{ ok: boolean, error?: string }}
 */
async function _sendViaResend(to, subject, text, html) {
  try {
    const { data, error } = await _resendClient.emails.send({
      from: FROM_ADDRESS,
      to:   [to],
      subject,
      text,
    ...
    if (error) {
      console.error(`❌ Resend 送信失敗 (${to}):`, error.message ?? JSON.stringify(error));
      return { ok: false, error: error.message ?? 'Resend error' };
    }
    console.log(`✅ Resend 送信成功: id=${data?.id} → ${to}`);
    return { ok: true };
  } catch (e) {
    console.error(`❌ Resend 例外 (${to}):`, e.message);
...
<div style="font-family: -apple-system, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; color: #484545;">
  ...
  <div style="background: #FDFBF8; border: 2px solid #209DDB; border-radius: 12px; padding: 20px; text-align: center; margin: 0 0 24px;">
    <div style="font-size: 32px; font-weight: bold; letter-spacing: 0.4em; color: #209DDB; font-family: 'SF Mono', Menlo, monospace;">
      ${_esc(code)}
    </div>
  </div>
  ...
  // Resend HTTP API（SMTP 不使用）
  _getTransporter(); // 初期化を走らせる
  if (_resendClient) {
    const r = await _sendViaResend(to, subject, text, html);
    return { ...r, devCode: IS_DEV ? code : undefined };
  }
  ...
  try {
    ...
    return { ok: false, error: e.message, devCode: IS_DEV ? code : undefined };
  }
}
...
<div style="font-family: -apple-system, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; color: #484545;">
  ...
  <div style="text-align: center; margin: 0 0 24px;">
    <a href="${_esc(resetUrl)}" style="display: inline-block; background: #209DDB; color: #fff; font-weight: bold; padding: 12px 28px; border-radius: 999px; text-decoration: none; font-size: 14px;">パスワードを再設定する</a>
  </div>
  ...
  if (_resendClient) {
    const r = await _sendViaResend(to, subject, text, html);
    return { ...r, devUrl: IS_DEV ? resetUrl : undefined };
  }
...
function logTransportStatus() {
  ...
  if (TRANSPORT_KIND === 'console') {
    ...
    console.log('   Render 本番環境では MAIL_TRANSPORT=resend を推奨（SMTP は Render でブロックされます）。');
  } else if (TRANSPORT_KIND === 'resend') {
    _getTransporter();
    if (_resendClient) {
      console.log(`   Resend HTTP API 送信元: ${FROM_ADDRESS}`);
    } else {
