...
async function _ensureIndexes(db) {
  ...
  await projects.createIndex({ 'members.userId': 1 }, { name: 'members_userId' });
  await projects.createIndex({ ownerId: 1 },           { name: 'ownerId' });
  await projects.createIndex({ createdAt: -1 },        { name: 'createdAt_desc' });
  ...
  await submissions.createIndex({ projectId: 1, missionId: 1 }, { name: 'project_mission' });
