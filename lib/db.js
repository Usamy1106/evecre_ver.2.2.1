...
// push 送信ログ（重複送信防止用）の TTL（秒）：30 日
const PUSH_LOG_TTL_SEC = 60 * 60 * 24 * 30;
...
async function connectDb() {
  ...
  _client = new MongoClient(MONGODB_URI, {
    ...
    // Render ↔ Atlas 間の通信を圧縮する（イベント1件の JSON は大きく、
    // 一覧の取得やブロードキャストで何度も往復するため）。
    // ★zlib は Node 内蔵。snappy / zstd は別のネイティブパッケージが要るので使わない
    //   （Render のビルドにネイティブ依存を増やさない方針。CLAUDE.md のアイコン生成と同じ判断）。
    // ★level は控えめ（既定は -1＝標準）。0.5CPU なので上げないこと。
    compressors: ['zlib'],
...
async function _ensureIndexes(db) {
  ...
  // ── push_subscriptions（Web Push の購読）─────
  // endpoint がブラウザ・端末ごとに一意。1ユーザーが複数端末を持つので userId は一意にしない。
  const push_subs = db.collection('push_subscriptions');
  await push_subs.createIndex({ endpoint: 1 }, { unique: true, name: 'endpoint_unique' });
  await push_subs.createIndex({ userId: 1 },   { name: 'userId' });

  // ── push_dispatch_log（重複送信の防止）───────
  // 「送信前に insert を試みて、重複エラーならスキップ」で冪等性を担保する。
  // 再起動時のキャッチアップ・手動実行・多重起動で同じ通知が飛ぶのを防ぐ。
  const push_log = db.collection('push_dispatch_log');
  await push_log.createIndex({ dedupeKey: 1 }, { unique: true, name: 'dedupeKey_unique' });
  await push_log.createIndex(
    { sentAt: 1 },
    { expireAfterSeconds: PUSH_LOG_TTL_SEC, name: 'push_log_ttl' }
  );

  // ── users（最終アクティビティ。idle 系の定期通知で使う）──
  await users.createIndex({ lastSeenAt: 1 }, { name: 'lastSeenAt', sparse: true });
