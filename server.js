...
app.use(require('express').static(require('path').join(__dirname, 'public'), {
  ...
  setHeaders: (res, filePath) => {
    ...
    // ★背景以外の画像（アイコン・キャラクター・エンプティステート・プリセットアバター）は
    //   既定（max-age=0 + ETag）のまま＝**毎回再検証する**。
    //   一度 max-age=86400 にしたが、すぐに「アイコンが古いまま出る」で戻した（2026-09-24）。
    //   これらは URL に版を持たないので、キャッシュを持たせると差し替え
    //   （npm run icons）の反映が遅れ、原因の分からない不具合に見える。
    //   削れるのは1画面あたり数本の 304 で、割に合わない。
    //   ★持たせたくなったら、先に /images/bg/ と同じ ?v=<ハッシュ> を付けること。
  },
}));
