...
- 書くのは素の CSS ＋ `@import` だけ。**分割の仕方も書き方も今までどおり**
- ただし**配信するのは連結した1本**（`style.bundle.css`）。理由と運用は下の「連結について」を読む
- `style.css` は**ソースの正**（`@import` の並び＝詳細度の順）。ブラウザはこれを読まない
...
| 文章の中の URL のリンク（アーカイブ） | `object/component/_text-link.css` |
| 文章の途中に画像・PDF を置ける編集欄（完了フォーム・提出内容の編集） | `object/component/_editor.css` |
| 「他の団体にも役立ちそう」の大きなチェック | `object/component/_share-check.css` |
...
### 連結について（`style.bundle.css`）

`index.html` が読むのは `style.bundle.css?v=<ハッシュ>` **1本だけ**。
`scripts/buildCss.mjs` が `style.css` の `@import` を順番に連結して作る。

- **なぜ**：`@import` 73本は「`style.css` を読む → 73本を取る」の2段になり、しかも
  全部 `no-cache` なので**毎回73本の 304 が飛ぶ**。回線が細い環境ではこれ自体が失敗の種になる。
  連結＋`?v=` の immutable 配信で、2回目以降は往復ごと消える
- **開発中は何もしなくてよい**。サーバーが要求のたびに作り直す（`IS_DEV` のときだけ）
- **コミットする前に `npm run css:build` を流す**。生成物はリポジトリに入れる。
  流し忘れは `npm run check:flocss` が中身を作り直して比べ、一致しなければ落とす
- ★**`style.bundle.css` を手で編集しないこと**（次の生成で消える）
- ★**`url()` は絶対パス（`/images/...`）で書くこと。** 相対パスは連結でベースがずれて
  画像が出なくなる（`check:flocss` が検査する）
...
│  ├─ _text-link.css            ✅ c-text-link（文章の中の URL）
│  ├─ _editor.css               ✅ c-editor / c-editor-field（clearEditor.js）
│  ├─ _share-check.css          ✅ c-share-check（Components.ShareCheck）
