...
// ★6（2026-09-19。以前は 14＝iPhone で約18マスごと）。
//   実測の中央値：375px 幅 7.4 マス／390px 7.7／430px 8.5／448px 8.9。
//   参考：8 にするとスマホでおよそ10マスごと（375px 9.9／390px 10.3）。
 ...
 * **絵の箱どうしが縦横とも重なったら true**（横は PLANT_GAP のすき間も込み）。
 *
 * ★2026-09-23 に厳密な判定へ変えた。それまでは「横に重なっても根元の高さが
 *   十分違えば別の段として許す」形（PLANT_ROW_RATIO）で、奥行きが出て本数も入る
 *   代わりに絵が重なって見えていた（重なるなという指摘を受けた）。
 *   ★実測では本数はほとんど減らない（6イベント×完了48件で 134 → 126 本）。
 * ★代償：入る本数が減る。テーマ設定の count を増やしても、置き場所が無ければ
 *   入らない（_plantingPlan が諦める）ので、密度の検査は期待値を計算している。
...
function _plantingPlan(eventId, parts, board) {
  ...
  //   1000 素材px ほど超えることがある。途中の地形ならそれで正しく、はみ出した分は
  //   一段上の地形が隠してくれる（稜線から覗いて自然に見える）。
  //   隠してくれる相手がいないのは**いちばん高い地形だけ**で、そこを超えると
  //   空に浮いて見える（実際にその報告を受けた）。
  // ★parts.at(-1) で代用しないこと。地形の高さは3種あってバラバラなので、
  //   いちばん上の地形が低いと**1つ下のほうが高く突き出す**（実測で 5.7% のイベント、
  //   差は最大 281 素材px）。最高点は必ず全パーツから取る。
  // ★山頂の看板の有無（isSummit）で変えないこと。開催が終わった瞬間に草木が
  //   消えると「全メンバーがいつでも同じ景色を見る」が崩れる。
  const ceiling = parts.length ? Math.max(...parts.map(p => p.y + p.h)) : 0;
  ...
  for (let i = 0; i < cands.length; i++) {
    ...
    for (let j = 0; j < picks.length; j++) {
      ...
      //   見える。「並んで見える」のはこの背景で一番避けたい失敗。
      // ★x の探索より**前**に弾くこと。後ろに置くと PLANT_TRIES を無駄に回すうえ、
      //   placed に積まれて他の草木の居場所まで奪う。
      if (part.y + y + h > ceiling) continue;

      // ★重ならない位置が見つかるまで引き直す。見つからなければその1本は諦める
      //   （無理に詰めると等分スロットと同じ「並んだ」見た目に戻る）。
      // ★重なりは**絵の箱どうし**で見る（縦横とも重なったら置かない）。
      let x = -1;
