...
export function renderProjectSettings(container) {
  const p = state.projects.find(x => x.id === state.selectedProjectId);
  if (!p) {
    state.setView('HOME');
    return;
  }
  const sec = state.projectSettingsScreen || (state.projectSettingsScreen = { members: null, loadingMembers: true });
  ...
  container.innerHTML = `
    <div class="flex flex-col min-h-screen bg-[#FDFBF8] page-transition">
      <header class="flex items-center px-6 py-4 bg-[#FDFBF8] sticky top-0 z-20 border-b border-[#E1DFDC]">
        ...
        <h1 class="heading-r font-bold text-[#484545]">プロジェクト設定</h1>
      </header>

      <main class="flex-1 px-6 pt-6 pb-24 space-y-8">
        ${_membersAvatarsSection(sec)}
        ${_projectManagementSection(p, sec)}
...
// セクション: プロジェクト管理
// =====================================================
function _projectManagementSection(p, sec) {
  ...
  return `
    <section>
      <h2 class="heading-rs font-bold text-[#484545] mb-3 border-b border-[#E1DFDC] pb-1.5">プロジェクト管理</h2>
      <div class="bg-white rounded-2xl shadow-sm border border-[#E1DFDC] divide-y divide-[#E1DFDC]">

        <!-- プロジェクト名 -->
        <div class="p-4">
          <p class="text-[10px] text-[#A7AAAC] font-bold mb-1">プロジェクト名</p>
          ${editingName ? `
          ...
          ` : `
            <div class="flex items-center justify-between">
              ...
              <button data-ps-edit="name" class="text-[11px] text-[#0CA1E3] font-bold px-3 py-1.5 active:opacity-50">変更</button>
            </div>
          `}
        </div>
        ...
        <!-- プロジェクトの説明 -->
        <div class="p-4">
          <p class="text-[10px] text-[#A7AAAC] font-bold mb-1">プロジェクトの説明</p>
          ${editingDesc ? `
          ...
          ` : `
            <div class="flex items-start justify-between gap-3">
              ...
              <button data-ps-edit="description" class="text-[11px] text-[#0CA1E3] font-bold px-3 py-1.5 active:opacity-50 whitespace-nowrap">変更</button>
            </div>
          `}
        </div>
        ...
        <div class="p-4">
          ...
          <div class="flex items-center justify-between">
            ...
            <button onclick="window._app.openCalendarModal('projectEdit')" class="text-[11px] text-[#0CA1E3] font-bold px-3 py-1.5 active:opacity-50">変更</button>
          </div>
        </div>
    ...
    if (isEditing && !isOwnerRow) {
      ...
      const checks = roles.filter(r => r.id !== 'owner').map(r => `
        <label class="flex items-center gap-2 py-1.5 cursor-pointer">
          ...
          ${r.canManage ? '<span class="text-[9px] text-[#0CA1E3] font-bold">管理可能</span>' : '<span class="text-[9px] text-[#A7AAAC] font-bold">管理不可</span>'}
        </label>`).join('');
      return `
      ...
      return `
        <div class="px-4 py-3 bg-[#FDFBF8]">
          ...
          <label class="flex items-center gap-2 mb-3 cursor-pointer">
            ...
            <span class="text-[12px] text-[#484545] font-bold">管理可能</span>
            <span class="text-[10px] text-[#A7AAAC] ml-auto">${isOwnerRole ? '(オーナーは常にON)' : 'プロジェクト管理・ミッション編集ができる'}</span>
          </label>
          <div class="flex gap-2">
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-[#E1DFDC] mb-3">
        <div class="px-4 pt-4 pb-2 flex items-center justify-between">
          <div>
            ...
            <p class="text-[10px] text-[#A7AAAC] mt-0.5">「管理可能」がONのロールはプロジェクト・ミッションを編集できます</p>
          </div>
          ${canMgr ? `
  ...
  return `
    <div class="px-4 py-3 bg-[#FDFBF8] border-t border-[#E1DFDC]">
      <input id="ps-role-new-name" value="${_esc(sec.roleAdding.name || '')}"
        placeholder="例: デザイナー、閲覧者など"
        class="input-field w-full px-3 py-2 text-[13px] focus:outline-none mb-2" maxlength="20">
      <label class="flex items-center gap-2 mb-3 cursor-pointer">
        <input id="ps-role-new-canmanage" type="checkbox" ${sec.roleAdding.canManage ? 'checked' : ''}>
        <span class="text-[12px] text-[#484545] font-bold">管理可能</span>
        <span class="text-[10px] text-[#A7AAAC] ml-auto">プロジェクト管理・ミッション編集</span>
      </label>
      <div class="flex gap-2">
...
function _bindEvents(p, sec) {
  ...
  document.getElementById('ps-name-save')?.addEventListener('click', async () => {
    ...
    if (!v) { alert('プロジェクト名を入力してください'); return; }
    p.name = v;
    await state.save();
    sec.editing = null;
    sec.draftValue = null;
  ...
  document.getElementById('ps-role-add-save')?.addEventListener('click', async () => {
    ...
    const r = await api.createRole(p.id, name, !!sec.roleAdding?.canManage);
    if (r.ok) {
  ...
  document.querySelectorAll('[data-ps-role-save]').forEach(el => {
    el.addEventListener('click', async () => {
      ...
      const r = await api.updateRole(p.id, id, { name: draft.name, canManage: draft.canManage });
      if (r.ok) {
  ...
  document.querySelectorAll('[data-ps-role-delete]').forEach(el => {
    el.addEventListener('click', async () => {
      ...
      const r = await api.deleteRole(p.id, id);
      if (r.ok) {
