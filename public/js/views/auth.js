  ...
  container.innerHTML = `
    <div class="flex flex-col min-h-screen bg-[#FDFBF8] page-transition">
      <main class="flex-1 px-6 pt-16 pb-8 flex flex-col">
        ...
        <p class="text-rs text-[#A7AAAC] text-center mb-6 font-bold">プロジェクトを保存・共有するために、まずはアカウントを作成してください</p>
  ...
  document.getElementById('ca-submit').onclick = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // サーバー側で招待自動受諾済み → 直接プロジェクト画面へ + 認証モーダル
        if (r.pendingProjectId) {
          console.log('[register] サーバーが招待を自動受諾 projectId=', r.pendingProjectId);
          await state._enterInvitedProject(r.pendingProjectId);
          state.render();
          return;
        }
  ...
  document.getElementById('lg-submit').onclick = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // サーバー側で招待自動受諾済み → 直接プロジェクト画面へ
        if (r.pendingProjectId) {
          console.log('[login] サーバーが招待を自動受諾 projectId=', r.pendingProjectId);
          await state._enterInvitedProject(r.pendingProjectId);
          state.render();
          return;
        }
