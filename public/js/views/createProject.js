...
// ステップ1: プロジェクト名・説明
// ステップ2: 開催日時
// ステップ3: 招待リンク発行（画面到達時に自動でプロジェクト作成＋招待リンク発行）
//
// 種は addProject 内でランダム自動選択（ユーザー選択は廃止）。
...
// ステップ1: プロジェクト名・説明
// =====================================================
export function renderCreateProjectInfo(container) {
  const canNext = !!state.draftProject.name;
        ...
        <div class="w-full space-y-6 mb-12">
          <div>
            ...
            <input type="text" placeholder="プロジェクト名を入力"
              value="${_esc(state.draftProject.name)}"
              oninput="window._app.updateDraftInfo('name', this.value)"
              class="input-field w-full px-5 py-4 focus:outline-none">
          </div>
          <div>
            <label class="heading-rs block mb-2 text-[#484545]">プロジェクトの説明 <span class="text-[#A7AAAC] text-[11px]">（任意）</span></label>
            <textarea placeholder="プロジェクトの説明を入力" rows="4"
              oninput="window._app.updateDraftInfo('description', this.value)"
              class="input-field w-full px-5 py-4 focus:outline-none resize-none">${_esc(state.draftProject.description)}</textarea>
          </div>
        </div>
...
export function renderCreateProjectDates(container) {
  ...
  const selected  = state.draftProject.dates;
      ...
      <main class="flex-1 px-6 pt-2 pb-12 flex flex-col page-transition items-center">
        ...
        <div class="mt-auto w-full max-w-sm space-y-3 pt-6">
          ...
          <button id="cp-dates-next" onclick="window._app.tryProceedFromDates()"
            class="btn-primary w-full py-5 heading-m font-bold shadow-lg" ${canNext ? '' : 'disabled style="opacity:.5"'}>次へ</button>
          <button onclick="window._app.setView('CREATE_PROJECT_INFO')"
            class="btn-secondary w-full py-4 heading-m font-bold text-[#484545]">戻る</button>
        </div>
      </main>
    </div>`;
...
function _bindCalendarDrag(container) {
  ...
  const setCell = (dateStr, on) => {
    const arr = state.draftProject.dates;
  ...
  const onDown = (clientX, clientY) => {
    ...
    const wasSelected = state.draftProject.dates.includes(dateStr);
    _drag.active = true;
...
// =====================================================
export function renderCreateProjectInvite(container) {
  const sec = state.createProjectInviteScreen || (state.createProjectInviteScreen = {});

  // この画面に着いた瞬間にプロジェクト作成＋招待リンク発行を開始
  // (まだ実行していなければ)
        ...
        <div class="mt-auto w-full max-w-sm space-y-3 pt-8">
          ...
          ${sec.inviteUrl ? `
            <button id="cpi-finish"
              class="btn-primary w-full py-5 heading-m font-bold shadow-lg">プロジェクト画面へ</button>
          ` : sec.error ? `
            <button id="cpi-retry"
              class="btn-primary w-full py-5 heading-m font-bold shadow-lg">もう一度試す</button>
          ` : ''}
          <button onclick="window._app.setView('CREATE_PROJECT_DATES')"
            class="btn-secondary w-full py-4 heading-m font-bold text-[#484545]" ${sec.creating ? 'disabled style="opacity:.5"' : ''}>戻る</button>
        </div>
      </main>
    </div>`;

  document.getElementById('cpi-finish')?.addEventListener('click', _finishAndGoToProject);
...
function _renderCreating() {
  return `
    <div class="w-full max-w-sm flex flex-col items-center py-12">
      ...
      <p class="text-[13px] text-[#484545] font-bold">プロジェクトを作成中…</p>
    </div>`;
}
...
function _renderShare(url) {
  return `
    <p class="text-[12px] text-[#0CA1E3] font-bold text-center mb-2">✓ プロジェクトを作成しました</p>
...
async function _createAndIssueInvite() {
  const sec = state.createProjectInviteScreen;
  try {
    const projectId = await state._createProjectAndReturnId();
    if (!projectId) {
      sec.creating = false;
      sec.error = 'プロジェクト名・説明・開催日時を確認してください';
      state.render();
      return;
    }
    ...
    const url = `${location.origin}/invite/${r.invite.token}`;
    sec.creating = false;
    sec.inviteUrl = url;
    sec.projectId = projectId;
    state.render();
...
function _finishAndGoToProject() {
  const sec = state.createProjectInviteScreen || {};
  const id = sec.projectId;
  state.createProjectInviteScreen = {};
  // draftProject もリセット
  state.draftProject = { name: '', description: '', dates: [], seedType: 'jack' };
  if (id) {
...
function _buildInviteText(url) {
  ...
  const projectName = draft.name || 'プロジェクト';
