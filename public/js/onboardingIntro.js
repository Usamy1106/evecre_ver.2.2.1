...
// リーダーがイベントを作った直後に、
//   ① 進め方（モーダル）
//   ② イベント主要機能の紹介（コーチマーク4連：残り日数／AI提案／やること一覧／作成ボタン）
//   ③ 「目的を定めよう」の促し（コーチマーク）
// を順に通す。
//
// ★タスク作成フォームのツールチップは**この流れに含めない**。
...
// タスク作成フォームのツアーは**ユーザー単位**で1回だけ（イベント単位ではない）。
...
/**
 * ★state.render() からのみ呼ぶこと。
 * 進行状態に応じて①または②を出す。③はタスク作成モーダルを開いた時、
 * ④はタスク保存後に別経路で開始する。
 */
...
// ── タスク作成フォームのツールチップ（初期オンボーディングとは独立）──
/**
 * タスク作成モーダルが開いた直後に呼ぶ（modals/mission.js 経由）。
 *
...
export function startMissionFormTour() {
  ...
  const ok = startTooltipTour({
    steps: [
      ...
      // ★「期日」「締切」と読める語を使わないこと。タスクのスケジュールは1日ではなく
...
const FEATURE_STEPS = [
  ...
  },
  {
    ...
    title: '作ったタスクは、ここに並ぶよ。',
    available: () => true,
  },
];

/**
 * タスクを保存してボードに戻った直後に呼ばれる（checkIntro 経由＝render 駆動）。
 *
...
export function showFeatureTour() {
  ...
  //   ③が出ない人（後から参加した管理者／目的タスクが一覧に無い）では
...
// ── ③ 「目的を定めよう」の促し ──────────────────────────────
/**
 * イベント作成時に自動生成される「このイベントの目的を定めよう」（def-1）を指し示す。
 *
...
export function showPurposeCoach() {
  ...
  const shown = showCoachMark({
    ...
    // ★穴の外をタップしたら、目的タスクを開かずに閉じる。
    //   出口が「目的タスクのタップ」だけだと、他を触りたい人が閉じ込められる。
  ...
  if (!shown) {
    // 目的タスクが一覧に無い（＝すでに完了している等）。止めずに完了させる
    _finishIntro();
    return;
  }
  setIntroRunning(true);
