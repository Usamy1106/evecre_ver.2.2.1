...
export const Components = {
  /**
   * メール認証バナー（未認証ユーザーのみ表示。HOMEとプロジェクト画面の両方で使う）
   */
  VerifyBanner() {
      ...
      <div class="mx-6 mt-2 mb-2 bg-[#FFF7E6] border border-[#FFC300] rounded-2xl p-4">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-[11px] text-[#484545] leading-snug">新規プロジェクトの作成など、一部の機能はメール認証完了まで使えません。</p>
  ...
  /**
   * ヘッダー
   * @param {object|null} project - プロジェクトオブジェクト（nullでホーム用ヘッダー）
   */
  Header(project) {
    if (!project) {
      ...
      return `
        <header class="flex justify-between items-center px-6 py-4 bg-[#FDFBF8] sticky top-0 z-20">
          ...
          <button onclick="${verified ? `window._app.setView('CREATE_PROJECT_INFO')` : `window._app.requireVerification()`}"
