...
const STEPS = [
...

  {
    ...
    build: () => ({
      ...
      // ★自動作成される「このイベントの目的を定めよう」が既に入っているので、
      //   「最初の1件を作ろう」は不正確。「作ってみよう」に留める。
      body: 'やることを書き出してスケジュールを作ってみましょう。',
      // ★ボタンは「わかった」だけ（2026-09-21）。押したら閉じるだけにし、
      //   タスクの作成モーダルは開かない。読み終えた直後に作成画面が出ると、
      //   何を書くか決まっていない人には手が止まる画面を押し付けることになる。
      //   ★action を戻さないこと／「あとで」を戻さないこと（選択肢を増やさない）。
      primary: 'わかった',
      dismissible: false,
    }),
  },
