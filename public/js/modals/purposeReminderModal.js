...
//      1イベントにつき一度きり。
//   B. 7日間タスクの作成・完了が無い（無活発）。以後さらに7日経過するたびに再表示。
// 判定は render() 駆動のみ（バックグラウンドタイマー厳禁。CLAUDE.md「タスク提案」節と同じ方針）。
...
// 直近のタスク活動時刻（作成 or 完了のうち最新）。タスク操作が一切無ければイベント作成時刻。
...
export function checkPurposeReminderModal() {
  ...
  // ★開催日を過ぎたイベントでは出さない。
  //   とくに条件B（無活発）は、開催後にタスクの動きが止まるのが当然なので
