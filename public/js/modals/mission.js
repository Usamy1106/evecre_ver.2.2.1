...
export function openMissionModal(missionId = null) {
  // 管理不可なユーザーは編集禁止
  if (!state.canManageCurrentProject()) {
...
export function renderMissionModalContent() {
  ...
  const dateDisplay = state.draftMission.dates.length > 0
    ? `<div class="bg-[#EBE8E5] px-3 py-1.5 rounded-lg text-[#484545] font-bold text-rs">${state.draftMission.dates[0]}</div>`
    : 'カレンダーから設定する';
...
function _renderBasicTab(isEdit, dateDisplay) {
  ...
  const formatButtons = ['text', 'image', 'link'].map(f =>
    `<div onclick="window._app.setMissionFormat('${f}')"
      ...
      <div class="w-12 h-12 border-2 rounded-lg flex items-center justify-center
        ${state.draftMission.clearFormat === f ? 'border-[#0CA1E3]' : 'border-[#A7AAAC]'}">
        <img src="/images/icon/icon-${f === 'text' ? 'TextBox' : f === 'image' ? 'image-color' : 'Link'}.svg" class="w-6 h-6">
      </div>
  ...
  return `
    <div class="flex flex-col h-full">
      ...
      <div class="space-y-4 flex-1">
        ...
        <div>
          <label class="heading-rs block mb-3 text-[#484545]">形式</label>
          <div class="flex justify-around">${formatButtons}</div>
        </div>
      </div>
...
function _renderDetailTab(isEdit) {
  ...
  const claimMode = state.draftMission.claimMode || 'first';
  ...
  return `
    <div class="flex flex-col h-full">
      ...
      <div class="space-y-6 flex-1">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="heading-rs text-[#484545] font-bold">チェック項目</label>
            <button onclick="window._app.addChecklistItem()"
              class="text-[12px] text-[#0CA1E3] font-bold px-3 py-1.5 active:opacity-50">+ 追加</button>
          </div>
          <p class="text-[11px] text-[#A7AAAC] mb-3">完了時にチェックしないと提出できません</p>
          ${checklist.length === 0 ? `
            <p class="text-[11px] text-[#A7AAAC] text-center py-3 bg-[#FDFBF8] rounded-xl border border-dashed border-[#D3D6D8]">
              「+ 追加」でチェック項目を作成
            </p>
          ` : `
            <div class="space-y-2">
...
export function showProposalHelp(e, proposalId) {
  e.stopPropagation();
  const desc = MISSION_DESCRIPTIONS[proposalId] || 'ミッションを完了してプロジェクトを進めましょう。';
...
export function toggleMissionMenu(e, missionId) {
  ...
  menu.innerHTML = `
    <button onclick="event.stopPropagation(); window._app.openMissionModal('${missionId}')"
      class="w-full text-left px-4 py-3 hover:bg-[#FDFBF8] text-rs font-bold border-b border-[#FDFBF8]">編集</button>`;
...
function _renderAssigneeSheetList() {
  ...
  if (tab === 'ACCOUNT') {
    ...
    if (roles.length === 0) {
      ...
      html += roles.map(r => row(`role:${r.id}`, r.name, r.canManage ? '管理可能' : '管理不可')).join('');
    }
  }
...
export function openSelectClaimModal(missionId) {
  ...
  const modeLabel = m.claimMode === 'multi' ? '複数人可' : '選定あり';
